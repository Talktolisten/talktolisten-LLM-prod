...
RUN python3.8 -m pip install torch==2.1.0
RUN python3.8 -m pip install transformers==4.35.2
RUN python3.8 -m pip install accelerate==0.25.0
RUN python3.8 -m pip install auto-gptq==0.6.0
RUN python3.8 -m pip install optimum==1.16.1
RUN python3.8 -m pip install huggingface-hub==0.20.2

COPY model_fetcher.py /src/model_fetcher.py
RUN python3.8 model_fetcher.py --model_name=${MODEL_NAME}

COPY infer.py /src/infer.py
COPY test_input.json /src/test_input.json

CMD python3.8 -u infer.py --model_name=${MODEL_NAME}